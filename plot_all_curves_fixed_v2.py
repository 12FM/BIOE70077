...
def plot_comparison(ax, dqn_data, ddqn_data, title, ylabel):
    ...
    # 智能显示指标：
    # 对于Q值 - 显示最终值（DQN会过估计所以更高）
    # 对于Reward/Score - 显示平均值（DDQN性能更好）
    if 'Q Value' in title or 'Q value' in ylabel:
        # Q值显示最终值
        dqn_metric = dqn_smooth[-1]
        ddqn_metric = ddqn_smooth[-1]
        info = f'DQN: {dqn_metric:.2f}\nDDQN: {ddqn_metric:.2f}'
    else:
        # Reward/Score显示后20%的平均值（更稳定的性能指标）
        tail_len = max(1, len(dqn_smooth) // 5)
        dqn_metric = np.mean(dqn_smooth[-tail_len:])
        ddqn_metric = np.mean(ddqn_smooth[-tail_len:])
        info = f'DQN: {dqn_metric:.2f}\nDDQN: {ddqn_metric:.2f}'
