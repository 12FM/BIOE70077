...
class DQNAgent:
    """
    Class for standard DQN model architecture (for comparison with DDQN).
    The key difference from DDQN: uses max Q from target network directly.
    """
    def __init__(self, env="BreakoutNoFrameskip-v4"):
        ...
        self.log_path = "./log/" + datetime.now().strftime("%Y%m%d_%H%M%S") + "_DQN_" + self.game_id
    ...
    def get_eps(self, current_step, terminal_eps=0.01, terminal_frame_factor=25):
        """Use annealing schedule for epsilon."""
    ...
    def update_main_q_network(self, state_batch, action_batch, reward_batch, next_state_batch, terminal_batch):
        """Update main q network - Standard DQN version.
        
        Key difference from DDQN:
        - DQN: uses max Q value from target network directly
        - DDQN: uses action from main network, Q value from target network
        """

        with tf.GradientTape() as tape:
            # Standard DQN: use max Q from target network directly
            q_target = self.target_network(next_state_batch)
            max_q_target = tf.reduce_max(q_target, axis=1)  # This is the key difference!
            expected_q = reward_batch + self.discount_factor * max_q_target * (1.0 - tf.cast(terminal_batch, tf.float32))
    ...
    def play(self, load_dir=None, episode=None, trial=5, max_playing_time=10):
        ...
        for _ in range(trial):
            ...
            while not done:
                ...
                if len(frames) > 15 * 60 * max_playing_time:
        ...
        imageio.mimsave("test_dqn.gif", frame_set[best_score_ind], fps=15)
